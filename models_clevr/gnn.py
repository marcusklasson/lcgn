...
class GNN(nn.Module):
    ...
    def build_propagate_message(self):
        ...
        self.queries = ops.Linear(cfg.CTX_DIM, cfg.CTX_DIM)
        self.keys = ops.Linear(cfg.CTX_DIM, cfg.CTX_DIM)
        self.vals = ops.Linear(cfg.CTX_DIM, cfg.CTX_DIM)
        #self.proj_keys = ops.Linear(cfg.CMD_DIM, cfg.CTX_DIM)
        #self.proj_vals = ops.Linear(cfg.CMD_DIM, cfg.CTX_DIM)
        self.mem_update = ops.Linear(cfg.CTX_DIM, cfg.CTX_DIM)
    ...
    def propagate_message(self, x_loc, entity_num):
        ...
        queries = self.queries(x_loc)
        keys = self.keys(x_loc) 
        vals = self.vals(x_loc) 
        edge_score = (
        ...
        x_loc_new = self.mem_update(message)
        return x_loc_new
